...
class escuela_controller:
    ...
    def modificar_escuela(escuela_uuid: str, update_data: escuela_update, db: Session = Depends(get_db)):
        try:
    ...
    def cambiar_estado_escuela(escuela_uuid: str, data: escuela_estado_update, db: Session = Depends(get_db)):
        try:
    ...
    @router.get("/", response_model=api_response[escuelas_paginadas], status_code=status.HTTP_200_OK)
    def listar_escuelas(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
        try:
            resultado = escuela_service.listar_escuelas(db, skip=skip, limit=limit)
            return api_response(
                code=status.HTTP_200_OK,
